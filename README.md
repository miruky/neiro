...
作った音色は名前を付けて手元に貯められるほか、すべての設定を短い共有リンク(URL)へ畳み込める。リンクを開いた相手はその音色がセットされた状態で始められる。JSONとして書き出し・読み込みもでき、思いつかないときはランダム生成から出発できる。表示はライト・ダークを切り替えられ、選択は次回も引き継がれる。
...
音を鳴らす部分(`engine.ts`)はAudioContextに依存するため、純粋な計算は分けてある。音名と周波数(`notes.ts`)、波形の標本化(`waveform.ts`)、フィルタの周波数特性(`filterResponse.ts`)、エンベロープの形(`adsr.ts`)、音色データの検証と保存(`patch.ts`)、共有リンクの符号化(`share.ts`)、ランダム生成(`random.ts`)、テーマの解決(`theme.ts`)、保存済み音色の管理(`userPresets.ts`)はDOMもWeb Audioも使わない純粋関数で、ブラウザなしでテストする。画面(`app.ts`)はこれらを束ねてエンジンと図へ反映する。
...
| 永続化・共有         | localStorage / URL(外部サービスなし) |
...
プリセットのボタンで、作り込んだ音色から出発できる。`ランダム` は音楽的に成り立ちやすい範囲で音色を組み直す。

### 音色を保存・共有する

| 操作       | 内容                                                                           |
| :--------- | :----------------------------------------------------------------------------- |
| 保存       | プリセット欄の入力に名前を付けて保存。この端末に残り、欄のチップから呼び出せる |
| 共有リンク | 現在の音色を畳み込んだURLをクリップボードへコピー。アドレス欄も常に最新を指す  |
| 書き出し   | 音色を `neiro-patch.json` としてダウンロード                                   |
| 読み込み   | 書き出したJSONを読み込んで復元                                                 |

共有リンクは音色の全パラメータを量子化した整数列(`#p=...`)で、外部サービスを介さずURLだけで完結する。リンクを開くと、保存済みの音色より共有リンクが優先される。
...
- 名前を付けた音色の保存はこの端末のブラウザに限られる。別の端末へ渡すには共有リンクかJSONを使う。
- 共有リンクは音色を量子化して畳むため、リンク経由で復元した値は元と極めて近いがビット単位では一致しないことがある。
...
- `src/lib/share.ts` — 音色と共有リンク(URL)の相互変換
- `src/lib/random.ts` — 決定的乱数とランダム音色の生成
- `src/lib/theme.ts` — テーマ選択の解決と永続化
- `src/lib/userPresets.ts` — 名前付き音色の保存・取り出し
...
- **状態は1つ** — 音色は単一のパッチオブジェクトに集約し、保存・復元・プリセット適用・共有リンクの読み書きを、その差し替えだけで扱う。
- **URLを状態の器にする** — 全パラメータを量子化してURLハッシュに畳み込み、バックエンドなしで音色を共有・ブックマークできるようにする。復元時は範囲外を必ず正規化し、壊れたリンクでも安全に倒れる。
